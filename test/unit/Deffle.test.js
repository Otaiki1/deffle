  ...
  : describe("Deffle Unit Tests", async () => {
      ...
      describe("create raffle", async () => {
        it("reverts when wrong inputs are passed", async () => {
          ...
          ).to.be.revertedWith("Error__CreateRaffle");
        ...
        it("successfully creates a raffle and emits the right events", async () => {
          await expect(
            ...
            .to.emit(deffle, "Deffle__RaffleCreated")
            .withArgs(1, addr2.address);
        });

        it("successfully creates a raffle and update state variables", async () => {
          const deffleEarningsBeforeRaffleCreation = fromEther(
            await deffle.deffleEarnings()
          );

          const txResponse = await deffle
            .connect(addr2)
            .createRaffle(
              correctStrData,
              correctEntranceFee,
          ...
          const deffleEarningsAfterRaffleCreation = fromEther(
            await deffle.deffleEarnings()
          );
          ...
          expect(deffleEarningsAfterRaffleCreation).to.be.gt(
            deffleEarningsBeforeRaffleCreation
          );
      ...
      describe("Enter a created raffle", async () => {
        ...
        it("reverts when wrong inputs are passed", async () => {
          ...
          await expect(
            deffle
              .connect(addr3)
              .enterRaffle(0, correctPassCode, { value: correctEntranceFee })
          ...
          await expect(
            deffle
              .connect(addr3)
              .enterRaffle(raffleId, wrongPassCode, { value: toEther("0.01") })
          ).to.be.revertedWith("Error__EnterRaffle");
          //Test for exceedng raffleId
          await expect(
            deffle
              .connect(addr3)
              .enterRaffle(3, correctPassCode, { value: toEther("0.01") })
          ).to.be.revertedWith("Error__EnterRaffle");
        ...
        it("successfully enters a raffle and updates state variables", async () => {
          ...
          const participantsBeforeEntry = await deffle.getNumberOfPlayers(
            raffleId
          );
          //check raffle balance before entry
          const raffleBalanceBeforeEntry = fromEther(
            (await deffle.getRaffleBalance(raffleId)).toString()
          );
          ...
          const raffleBalanceAfterEntry = fromEther(
            (await deffle.getRaffleBalance(raffleId)).toString()
          );
      ...
      describe("performUpkeep", function () {
        ...
        it("can only run if checkupkeep is true", async () => {
          await deffle.enterRaffle(raffleId, correctPassCode, {
            value: correctEntranceFee,
          });
          await increaseTime();
      ...
      describe("fulfillRandomWords", function () {
        ...
        it("updates state variables and make payouts", async () => {
          const raffleBalanceBeforeFulfill = await deffle.getRaffleBalance(
            raffleId
          );
          const ownerBalanceBeforeFulfill = await ethers.provider.getBalance(
            addr2.address
          );
          const winnerBalanceBeforeFulfill = await ethers.provider.getBalance(
            addr3.address
          );
          ...
          const raffleBalanceAfterFulfill = await deffle.getRaffleBalance(
            raffleId
          );
          const ownerBalanceAfterFulfill = await ethers.provider.getBalance(
            addr2.address
          );
          const winnerBalanceAfterFulfill = await ethers.provider.getBalance(
            addr3.address
          );
          const raffleState = await deffle.getRaffleState(raffleId); // stores the new state
          ...
          expect(raffleBalanceBeforeFulfill).to.be.gt(
            raffleBalanceAfterFulfill
          ); // the balance should have reduced
          expect(ownerBalanceBeforeFulfill).to.be.lt(ownerBalanceAfterFulfill); //balance should have increased
          expect(winnerBalanceAfterFulfill).to.be.gt(
            winnerBalanceBeforeFulfill
        ...
        it("emits the right events upon successful fulfill", async () => {
          ...
          await expect(
            vrfCoordinatorV2Mock.fulfillRandomWords(requestId, deffle.address)
          )
            .to.emit(deffle, "Deffle__WinnerPicked")
      ...
      describe("Withdraw Contract earnings", function () {
        ...
        it("only allows owner to wthdraw deffle earnings", async () => {
          await expect(
            deffle.connect(addr3).withdrawDeffleEarnings()
        ...
        it("Updates state Variables and makes payouts", async () => {
          ...
          //they should change
          expect(contractBalanceAfterWithdrawal).to.be.lt(
        ...
        it("Should emit right events", async () => {
          ...
          await expect(deffle.withdrawDeffleEarnings())
            .to.emit(deffle, "Deffle__EarningsWithdrawn")
